...
function PassengerPage() {
  ...
  const { alerts, triggered, dismissTriggered, isAlertOn, turnOn, turnOff, isDemo, exitDemo } = useBusAlerts();
  ...
  // Alerts about to trigger the banner/notification, from two sources:
  // 1. The backend's real GPS-proximity check (process_live_location) — the
  //    bus has been confirmed within ~500m of the stop. This is authoritative.
  // 2. A client-side ETA guess, kept as a fallback for cases the backend
  //    trigger hasn't caught yet (e.g. the alerted stop isn't the bus's
  //    immediate "next stop" in the RPC's ordering, but ETA still looks close).
  const arriving = useMemo(() => {
    const fromBackend = triggered.map((t) => ({
      key: `${t.busId}:${t.stopId}`,
      busId: t.busId,
      stopId: t.stopId,
    ...
    const fromEta = alerts.flatMap((a) => {
    ...
    return [...fromBackend, ...fromEta].filter((a) => {
      if (seen.has(a.key)) return false;
      seen.add(a.key);
      return true;
    });
  }, [triggered, alerts, allBuses]);
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 z-[450] p-3 sm:inset-y-0 sm:right-auto sm:w-[24rem] sm:overflow-y-auto">
          <div className="pointer-events-auto mx-auto max-w-3xl space-y-2 sm:max-w-none">
            {arriving.map((a) => (
              <div
                key={a.key}
                className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-2 rounded-2xl border-2 border-success bg-success/15 px-4 py-3 shadow-[var(--shadow-panel)]"
              >
                ...
                <button
                  onClick={() => dismissTriggered(a.busId, a.stopId)}
                  className="shrink-0 rounded-lg px-2 py-1 text-xs font-bold uppercase text-success hover:bg-success/20"
                >
                  Dismiss
                </button>
              </div>
